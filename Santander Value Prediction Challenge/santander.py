...
train_X = percent.fit_transform(train_X, train_y)
test_X = percent.transform(test_X)

estimator = ExtraTreesRegressor(n_estimators=200, max_depth=20, max_features=0.5, n_jobs=-1, random_state=0)
pipe_select_from_model = Pipeline([('scaler', RobustScaler()),
                                       ('reduce_dim', SelectFromModel(estimator)),
                                       ])
pipe_select_from_model.fit(train_X, train_y)
...
log_transformation.fit(train_X_reduced)
train_X = log_transformation.transform(train_X)
...
model_GBoost = GradientBoostingRegressor(n_estimators=150, 
                                         learning_rate=0.05)
...
model_extra = ExtraTreesRegressor(n_estimators = 150, 
                                  max_features=0.5)
