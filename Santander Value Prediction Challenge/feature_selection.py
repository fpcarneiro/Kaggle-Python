...
def get_feature_importance(estimator, X, y):
    ...
    if hasattr(pipe_importance.named_steps['estimator'], 'coef_'):
...
def remove_features_percentile(scaler, X, y, percentile = 50):
    pipe_percentile = Pipeline([('scaler', scaler),
                                ('reduce_dim', SelectPercentile(mutual_info_regression, percentile=percentile)),
                                ])

    pipe_percentile.fit(X, y)
